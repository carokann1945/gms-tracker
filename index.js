...
async function main() {
  ...
  if (!top10.length) {
    console.log('[main] No event items found. Skipping event pipeline.');
  } else {
    ...
    if (!newItems.length) {
      console.log('[main] No new items to process.');
    } else {
  ...
  // 6. Run maintenance pipeline (이벤트 파이프라인 결과와 무관하게 항상 실행)
